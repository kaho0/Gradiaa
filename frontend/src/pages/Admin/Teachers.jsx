...
const Teachers = () => {
  ...
  const fetchTeachers = async () => {
    try {
      const response = await axios.get(`${import.meta.env.VITE_BASE_URL}/teachers/getall`);
  ...
  const handleAddTeacher = async (e) => {
    ...
    if (newTeacher.name.trim() !== '' && newTeacher.email.trim() !== '' && newTeacher.subject.trim() !== '') {
      try {
        const response = await axios.post(`${import.meta.env.VITE_BASE_URL}/teachers`, newTeacher);
